...
class pmos(DloGen):
    ...
    def genLayout(self):
        ...
        ndiff_layer = Layer('Activ')     # 1
        pdiff_layer = Layer('Activ')     # 1
        poly_layer = Layer('GatPoly')    # 5
        poly_layer_pin = Layer('GatPoly', 'pin')
        locint_layer = Layer('Cont')     # 6
        metall_layer = Layer('Metal1')   # 8
        pdiffx_layer = Layer('pSD')      # 14
        well_layer = Layer('NWell')      # 31
        tgo_layer = Layer('ThickGateOx') # 44
